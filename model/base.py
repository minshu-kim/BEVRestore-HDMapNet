...
class BevEncode(nn.Module):
    def __init__(self, inC, outC, finetune=False, instance_seg=True, embedded_dim=16, direction_pred=True, direction_dim=37):
        ...
        for (imin, imax, _), (omin, omax, ostep) in zip( # BEV Crop at 0.6m/px scale
            [[-16.8, 16.8, 0.6], [-31.2, 31.2, 0.6]], [[-15.0, 15.0, 0.6], [-30.0, 30.0, 0.6]]
        ):
        ...
        self.grid = torch.stack([v, u], dim=-1)
    ...
    def forward(self, x):
        ...
        if self.finetune:
            grid = torch.stack([self.grid] * x.shape[0], dim=0).to(x.device)
            x = F.grid_sample(
                x,
                grid,
                mode="nearest",
        ...
        if self.instance_seg:
            ...
            if self.finetune:
                x_embedded = F.grid_sample(
                    x_embedded,
                    grid,
                    mode="nearest",
                    align_corners=False,
                )
                x_embedded = self.embed_sr_(x_embedded)
            x_embedded = self.embed_decoder(x_embedded)
        else:
            x_embedded = None
